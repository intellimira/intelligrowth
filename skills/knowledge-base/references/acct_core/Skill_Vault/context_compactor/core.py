...
def compact_context(folder_path):
    ...
    print(f"\n--- ACCT CONTEXT COMPACTOR: {os.path.basename(folder_path)} ---")
    ...
    summary_uid = "COMPACTED-" + files[0].split("_")[1] if "_" in files[0] else "SUMMARY"
    summary_path = os.path.join(folder_path, f"{summary_uid}.md")
...
if __name__ == "__main__":
    target = sys.argv[1] if len(sys.argv) > 1 else "/home/sir-v/ACCT_CORE/Memory_Mesh/History_Tree"
