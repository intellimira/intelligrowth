...
def extract_fields_from_email(body):
    ...
    for line in lines:
        ...
        elif line_lower.startswith("interest:") or line_lower.startswith(
            "i'm interested in"
        ):
    ...
    try:
        ...
        if password:
            ...
            if os.path.exists(token_file):
                ...
                mail.authenticate(
                    "XOAUTH2",
                    lambda x: f"user={GMAIL_USER}\1auth=Bearer {token}\1\1".encode(),
                )
            else:
                print("⚠️ No Gmail password configured.")
                print("   Set HIMALAYA_PASSWORD in ~/.env/mira_config")
                print(
                    "   Or generate an App Password from: https://myaccount.google.com/apppasswords"
                )
                return None
        return mail
...
def poll_for_enquiries():
    ...
    try:
        ...
        status, messages = mail.search(
            ...
            "FROM",
            "@gmail.com",  # Exclude our own emails
            "SUBJECT",
            "[,]",  # Emails with [CATEGORY] in subject
        )
...
def save_enquiries(enquiries):
    ...
    # Push to GitHub
    commit_msg = f"Auto: New enquiries {datetime.now().strftime('%Y-%m-%d %H:%M')}"
    os.system(
        f"cd {repo_path} && git add -A && git commit -m '{commit_msg}' && git push origin main 2>/dev/null"
    )
